/**
 * Menu item photos keyed by dish-name keywords — never random / hash-picked.
 * Keep in sync with baby-codes/scripts/lib/menu-item-images.mjs
 */
...
type DishPhotoKey = keyof typeof DISH_PHOTOS;

/** Most-specific rules first. */
const NAME_RULES: Array<{ keys: string[]; photo: DishPhotoKey }> = [
...
/**
 * Prefer a real partner upload. Stock/Unsplash dummy URLs are ignored so the
 * photo always matches the dish name (never a leftover random pool image).
 */
export function resolveMenuItemImage(
  name: string,
  imageUrl?: string | null,
): string {
  // Inline stock check — avoid importing media.ts (keeps this module script-safe).
